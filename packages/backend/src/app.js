...
// Migration: Add priority column to existing tables if it doesn't exist
try {
  db.exec(`ALTER TABLE tasks ADD COLUMN priority TEXT DEFAULT 'P3' CHECK(priority IN ('P1', 'P2', 'P3'))`);
  console.log('Added priority column to existing tasks table');
} catch (error) {
  // Column already exists or other error, this is expected
  console.log('Priority column already exists or migration not needed');
}

// Ensure existing tasks have priority set to P3 if NULL
db.exec(`UPDATE tasks SET priority = 'P3' WHERE priority IS NULL`);
...
app.post('/api/tasks', (req, res) => {
  try {
    ...
    // Validate priority if provided, default to P3
...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    // Validate priority if provided, keep current if not provided
    const validPriority = ['P1', 'P2', 'P3'].includes(priority) ? priority : 'P3';
